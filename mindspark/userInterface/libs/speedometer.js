...
function drawOuterMetallicArc(options) {
	...
	// Nice shade of grey
	options.ctx.fillStyle = "rgb(0,101,227)";
...
function drawSmallTickMarks(options) {
	...
	for (iTick = 10; iTick < 180; iTick += 15) {
		...
		line = createLine(fromX, fromY, toX, toY, "rgb(0,0,0)", 3, 0.6);
...
function drawSpeedometerColourArc(options) {
	...
	var startOfBlue = 20,startOfGreen = 140,
	    endOfGreen = 260,
	    endOfOrange = 320;

	drawSpeedometerPart(options, 1.0, "rgb(29, 188, 158)", startOfBlue);
	drawSpeedometerPart(options, 1.0, "rgb(153, 252, 3)", startOfGreen);
	drawSpeedometerPart(options, 0.9, "rgb(255, 215, 0)", endOfGreen);
	drawSpeedometerPart(options, 0.9, "rgb(0, 0, 255)", endOfOrange);
...
function convertSpeedToAngle(options) {
	...
	*/
	var iSpeed = (options.speed / 10),
	    iSpeedAsAngle = ((iSpeed * 15) + 10) % 180;
...
function drawNeedle(options) {
	...
	*/
	...
	drawNeedleDial(options, 0.6, "rgb(0,101,227)", "rgb(255,255,255)");
	drawNeedleDial(options, 0.2, "rgb(0,101,227)", "rgb(0,101,227)");
...
//		drawBackground(options);
		...
		//return;
		//alert("Canvas not supported by your browser!1111");
	}
	if ($('#dailyDrillReportImg').length>0){
		$('#dailyDrillReportImg').attr('src',canvas.toDataURL());
	}
...
function drawWithInputValue(inputvalue) {
	var txtSpeed = inputvalue;
	...
	if (txtSpeed !== null) {
		...
		if (isNaN(iTargetSpeed)) {
			...
			iTargetSpeed = 0;
		} else if (iTargetSpeed > 110) {
			iTargetSpeed = 110;
        }

        job = setTimeout("draw()", 500);
    }
}