...
window.onload=function()
{
  if(window.location.hash.length == 0)
  {
	...
	var commentSrNo = document.getElementById('commentSrNo').value;
	var theme = document.getElementById('theme').value;
	params="qcode="+qcode+"&mode=firstQuestion&quesCategory="+category+"&qno="+qno+"&tmpMode="+tmpMode+"&commentSrNo="+commentSrNo+"&theme="+theme;
...
function isNextQuesLoaded()
{
	...
	else
	{
		...
		{
			if(window.opener.showDivsAfterQuesAttempt)
			{
				var qcode = $("#qcode").val();
...
function showNextQuestion()
{
	...
	if(document.getElementById("tmpMode").value != "practice" && document.getElementById("tmpMode").value != "NCERT")
	{
		...
		if(objQuestion.quesVoiceOver!='')
			$("#quesVoiceOver").val('http://mindspark-ei.s3.amazonaws.com/'+objQuestion.quesVoiceOver);
		...
		{
			showSubmitButton();
			if (qType=='I' && /\/ADA_eqs\/src\/index\.html/.test(Q2)){
				$("#submitQuestion").hide();
				$("#submit_bar").hide();
				$("#submit_bar1").hide();
				$("#submitArrow").hide();
...
function submitAnswer()
{
	...
	if(document.getElementById("tmpMode").value != "practice" && document.getElementById("tmpMode").value != "NCERT")
	{
		...
		else if (objQuestion.quesType== 'Blank')
		{
            ...
            var result = objQuestion.checkAnswerBlank(userAnswer.split("|"), fracboxCheck,true);
...
function showAnswer(result, ques_type,  timeTaken)
{
	var msg;
	var frustrationMsg;
	var correctanswer = decrypt(objQuestion.enA);
	try{
	...
	doActionOnSubmitAnswer($("#qcode").val());
}
try {
	...
	eventer(messageEvent,function(e) {
		...
		if(response.indexOf("||") !=-1)
		{
			...
			if (typeof(responseArray[3]) !== 'undefined') {
				dispalAnswerParam = responseArray[3];
			}
			if(result>2)
			{
...
function evalGroupAnswers()
{
	...
	for (var k = 0; k < questionCount; k++)
    {
        ...
        {
			var result = objQuestion[k].checkAnswerBlank(userAns[k].split("|"),true, fracboxCheck[k]);
...
function decrypt(str) {
	var strtodecrypt = str.split("-");
	var msglength = strtodecrypt.length;
	decrypted_message = "";if (str=="") return decrypted_message;
	for (var position = 0; position < msglength; position++)        {
		ascii_num_byte_to_decrypt = strtodecrypt[position];
		ascii_num_byte_to_decrypt = ascii_num_byte_to_decrypt / 2;
		ascii_num_byte_to_decrypt = ascii_num_byte_to_decrypt - 5;
...
function CheckIfNumeric(evt) {
    ...
    var charCode = (/Firefox/.test(window.navigator.userAgent)) ? evt.which : evt.keyCode;
...
function getImportantQues()
{
	if(confirm("You will get only important questions"))
	{
		...
		var qno = document.getElementById('qno').value;
		var commentSrNo = document.getElementById('commentSrNo').value;
		params="qcode=0&mode=firstQuestion&quesCategory="+category+"&qno="+qno+"&impQuesMode=1&commentSrNo="+commentSrNo;
		getNextQues(params,"normal");
		fetchNextQues();
	}
}
