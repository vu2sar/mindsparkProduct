...
setformfieldsize.showlimit=function($field){    
	...
	if ($field.data('$statusdiv')){
	   var charsRemaining = $field.data('maxsize')-$field.val().length;
		$field.data('$statusdiv').css('color', '').html(charsRemaining + " characters left");
		var pctremaining=charsRemaining/$field.data('maxsize')*100 //calculate chars remaining in terms of percentage
