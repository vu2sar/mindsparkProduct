<?php
    ...
    {
        header("Location: controller.php?mode=logi	n");
    }
...
<!--	<script src="libs/closeDetection.js"></script>-->
	...
	<?php if($theme==3) { ?>
		...
		function trick_topic_load(idd)
		{
			...
			setTryingToUnload();
			document.getElementById('mtrcks').submit();
		...
		function load(){
			...
			$('#mtrcks').css({"height":b+"px"});
			$('#main_bar').css({"height":a+"px"});
			$('#menu_bar').css({"height":a+"px"});
			$('#sideBar').css({"height":a+"px"});
			if(androidVersionCheck==1){
				$('#mtrcks').css("height","auto");
				$('#main_bar').css("height",$('#mtrcks').css("height"));
				$('#menu_bar').css("height",$('#mtrcks').css("height"));
				$('#sideBar').css("height",$('#mtrcks').css("height"));
...
<body  class="translation"  onresize="load()" onLoad="">
	<div id="top_bar">
		<div class="logo">
		</div>
	...
	<div id="container">
		...
		<div id="info_bar" class="forHighestOnly">
				<div id="dashboard">
					...
					<!--<a href="explore.php">-->
					<span class="textUppercase" data-i18n="homePage.explore"></span></div>
					<!--</a>-->
                </div>
	...
	<form name="mtrcks" id="mtrcks" method="POST" action="trick_topic_load.php">
	<div id=mathTricks >
	<?php 
...
<?php } ?>
	...
	<div id="more_maths" >
	<div id = "more_maths_text"> Learn more maths shortcuts ?
	
	<input id="more_button" type="button" value="YES" onClick="explore_more_tricks()"> </input>
