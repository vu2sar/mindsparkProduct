...
//get the value of length
	...
	$light_grey=imagecolorallocate($image, 182,180,134) ;
	...
	$x3Point = 90+$first*cos(deg2rad( $deg3 ));      // cos 90 =0
	$y3Point = 90+$first*sin(deg2rad( $deg3 ));
	...
	imagearc($image, 90, 90, $first*2, $first*2, 0, $deg1, $black);
	imagearc($image, 90, 90, $first*2, $first*2, $deg2, 0, $black);
	
	imagearc($image, 90, 90, $first*2, $first*2, $deg1, $deg2, $red);
	...
	imagesetpixel($image, $x3Point,$y3Point, $blue);
	...
	$angle='&#60;';
	$angle=$deg2-$deg1;
	imagestring($image,7, $x1Point, $y1Point, "P", $black); //top
	imagestring($image,7, $x3Point, $y3Point, "Q", $black); //top
	imagestring($image,7, $x2Point, $y2Point, "R", $black); //top
	
	//imagestring($image,4, 120, 10, "pqr=".$angle, $black); //top
	
	imageline($image,90,90,$x1Point,$y1Point,$black);
	imageline($image,90,90,$x2Point,$y2Point,$black);
