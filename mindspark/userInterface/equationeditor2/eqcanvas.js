...
function ev_canvas (ev) {
    ...
    ev._x = ev.clientX - canvasClientRect.left;
    ev._y = ev.clientY - canvasClientRect.top;
...
tools.text = function () {
  ...
  tools.eraser = function () {
    ...
    this.mousedown = function (ev) {
        tool.started = true;
		//alert(this);
		context.strokeStyle="#000000"; 	context.lineWidth=1;context.beginPath();
		contexto.clearRect(ev._x-3*strokeWid, ev._y-3*strokeWid, 6*strokeWid, 6*strokeWid);
		context.clearRect(0, 0, canvas.width, canvas.height);
      	context.rect(ev._x-3*strokeWid, ev._y-3*strokeWid, 6*strokeWid, 6*strokeWid);
    ...
    this.mouseup = function (ev) {
      if (tool.started) {
        ...
        tool.started = false;
		context.clearRect(0, 0, canvas.width, canvas.height);
		context.strokeStyle=strokeCol; 	context.lineWidth=strokeWid;
        img_update();
      }
    };
  };
...
function getImageData(){
	var base64 = canvaso.toDataURL("image/png");
	return base64;
}
