...
class timedTestQuestion
{
	...
	function timedTestQuestion($qcode,$quesNo)
	{
		$query = "SELECT qcode, question, correct_answer, question_type, option_a, option_b, option_c, option_d,dynamic
				  FROM adepts_timedTest_questions 
				  WHERE  qcode=$qcode";
		...
		$this->optionA        = $line['option_a'];
		$this->optionB        = $line['option_b'];
		$this->optionC        = $line['option_c'];
		$this->optionD        = $line['option_d'];
		...
		/*$tmpHint = str_replace("&nbsp;","",strip_tags($this->hint));
		$tmpHint = str_replace("&#13;&#10;","",strip_tags($tmpHint));
		if($tmpHint=="")
			$this->hint = "";
	    ...
	    if($this->quesType=='D')
        {
        	$tmpArray = convertDropDowns($this->questionStem,$quesNo);
	...
	function getQuestion($quesNo)
	{
	    ...
	    if($this->country=="US" && $this->context!="US")   //Apply auto-replacement to questions which are not already marked for US content and the user's country is US
	       $context = "US";
		return orig_to_html($this->questionStem,"images","Q", $context,$quesNo);
	}
