...
class topicProgress
{
	...
	function getAccuracy($userID, $tillDate="")
	{
		$accuracy = "";
		$clusterStr = "'".implode("','",$this->clusterArray)."'";
		$clusterAttemptIDs = "";
		$clAttemptQuery = "SELECT clusterAttemptID FROM ".TBL_CLUSTER_STATUS." a, ".TBL_TOPIC_STATUS." b WHERE  b.ttAttemptID=a.ttAttemptID AND b.userID=a.userID AND b.userID=$userID AND a.clusterCode IN ($clusterStr) AND b.teacherTopicCode='".$this->objTT->ttCode."'";
		...
		{
			$sql = "SELECT SUM(R) as corrects, COUNT(R) as attempts FROM ".TBL_QUES_ATTEMPT."_class".$this->class." WHERE clusterAttemptID IN ($clusterAttemptIDs)";
			if($tillDate != "")
				$sql .= " AND attemptedDate <= '$tillDate'";
	...
	function getTotalQuestionAttempted($userID, $tillDate="")
	{
	...
	function getHigherLevel($userID, $tillDate="")
	{
		$this->higherLevel = 0;
		$maxCluster = "";
		if($this->clusterStr!="")
		{
			...
			if($tillDate!="")
			{
				...
				$tableName = ", ".TBL_SESSION_STATUS." C ";
				$whereClause = " AND B.startSessionID=C.sessionID AND startTime_int <= $tillDate ";
			}
			$sq = "SELECT clustercode FROM ".TBL_TOPIC_STATUS." A, ".TBL_CLUSTER_STATUS." B $tableName
				   WHERE A.userID=$userID AND teacherTopicCode='".$this->objTT->ttCode."' AND A.ttAttemptID=B.ttAttemptID $whereClause
				   ORDER BY FIELD(clusterCode, ".$this->clusterStr.") DESC LIMIT 1";
		    $rw = mysql_query($sq) or die("error:  $sq  ".mysql_error());
		    $rs = mysql_fetch_array($rw);
		    $maxCluster = $rs[0];
		    if($maxCluster!="" && !in_array($maxCluster,$this->clusterArray))
		    {
				$maxClusterLevel = $this->objTT->getClusterLevel($maxCluster);
		    	if($maxClusterLevel[0]>$this->objTT->startingLevel)    //implies gone to a higher level
		    		$this->higherLevel = 1;
