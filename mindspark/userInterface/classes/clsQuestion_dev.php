...
class Question
{
	...
	function Question($qcode)
	{
		
		if(strpos($qcode,"T") !== false || $_SESSION["modified"]==1)
		{
			$tbl_name	=  "adepts_quesUnlive";
			$tblCondition = "adepts_conditionalAlertUnlive";
			$tblName = "adepts_hintsUnlive";
		}
		else{
			$tbl_name	=  "adepts_questions";
			$tblCondition = "adepts_conditionalAlert";
			$tblName = "adepts_hints";
		}
		...
		$this->qcode          = $qcode;
		$arrayHint	=	$this->getHints($tblName);
		...
		$quesConditionArray	=	$this->getAllConditions($tblCondition);
		$quesActionArray	=	$this->getAllActions($tblCondition);
	...
	function getHints($tblName="adepts_hints")
	{
		$arrayHint	=	array();
		$sq	=	"SELECT hint,hintNo FROM $tblName WHERE qcode='".$this->qcode."'";
		if($_SESSION["modified"]!=1)
			$sq	.=	" AND status='Live'";
		$sq	.=	" ORDER BY hintNo";	
		$rs	=	mysql_query($sq);
	...
	function getAllConditions($tblCondition="adepts_conditionalAlert")
	{
		$quesConditionArray	=	array();
		$sq	=	"SELECT quesCondition,srNo FROM $tblCondition WHERE qcode='".$this->qcode."'";
		$rs	=	mysql_query($sq);
		$i=1;
		while($rw=mysql_fetch_array($rs))
		{
	...
	function getAllActions($tblCondition="adepts_conditionalAlert")
	{
		$quesActionArray	=	array();
		$sq	=	"SELECT quesAction,srNo FROM $tblCondition WHERE qcode='".$this->qcode."'";
		$rs	=	mysql_query($sq);
		$i=1;
		while($rw=mysql_fetch_array($rs))
		{
	...
	function replaceSwfWithHtml($html_ver,$queSwfFileName,$swfToHtmlRepStr)
	{
		preg_match_all("/\[($queSwfFileName.swf[^]]*)\]/i",$html_ver,$matches, PREG_SET_ORDER);
		$cnt_matches = count($matches);
		for($i=0 ; $i<$cnt_matches; $i++)
			$html_ver = str_replace($matches[$i][1],$swfToHtmlRepStr,$html_ver);	
