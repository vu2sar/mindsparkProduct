...
(function($) {
	...
	var inputosaurustext = {
		...
		options: {
			...
			// flag which will show first option by default even if there are multiple options
			showFirstOptionByDefault : false,

			// flag will be true if the automcomplete list which does not have <a> tag under <li> tag
			hasAnchorTag : false,
		},
			...
			if(this.options.autoCompleteSource){
				...
				this.elements.input.autocomplete({
					...
					select : function(ev, ui){
						ev.preventDefault();
						if(widget.options.hasAnchorTag)
							widget.elements.input.val(ui.item.label);
						else
					...
					open : function() {				
						...
						if(widget.options.activateFinalResult){
							...
							if(widget.options.showFirstOptionByDefault)
							{
								menu[menu.activate ? 'activate' : 'focus']($.Event('click'), $menuItems);
							}
						}
		...
		parseInput : function(ev) {
			...
			if(widget.options.hasAnchorTag)	// for the kudos autocomplete list which does not has <a> tag under <li>
			{
			...
			if(widget.options.hasAnchorTag)
			{
				switch(ev.which){
					...
					case $.ui.keyCode.ENTER:
						if(ev.type === 'keydown' && $(".ui-state-focus").size() == 0)
						{
