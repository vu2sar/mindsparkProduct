...
Class Endsessionreport_model extends MY_Model
{
	...
	function getEssayAttemptedInfo($userID,$startDate,$endDate)
	{
		...
		$this->dbEnglish->select('C.topicID,C.userResponse,B.essayTitle,C.essayID');
		$this->dbEnglish->from('essayMaster B, essayDetails C');	 
		$this->dbEnglish->where('C.userID',$userID);
		$this->dbEnglish->where('C.topicID = B.topicID');
		//$this->dbEnglish->where('A.ews_essayDetailsID = C.essayID');

		if($this->category == 'School Admin' || $this->category == 'TEACHER' || $this->category == 'ADMIN')
		{
			$this->dbEnglish->where("date(C.lastModified) >= '".date('Y-m-d',strtotime($startDate))."'");
			$this->dbEnglish->where("date(C.lastModified) <= '".date('Y-m-d',strtotime($endDate))."'");
		}
		else
		{
			$this->dbEnglish->where("date(C.lastModified) = '".$startDate."'");	
		}
		...
		foreach ($essayAttempt as $key => $valueArr) 
		{
			...
			if($this->isEssaySubmitted($userID,$valueArr['topicID'],$valueArr['essayID']) == 1)
