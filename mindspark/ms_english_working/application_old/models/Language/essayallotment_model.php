...
Class Essayallotment_model extends MY_Model
{
	...
	public function getEssaysToReview($userID,$childclass,$childsection){
		...
		$getReviewEssayIDsSql = "SELECT * FROM essayScoring e,userDetails u WHERE e.userID<>0 and e.evaluatorID = $userID and e.status=0 and u.userID=e.userID";
		if($childclass!='ALL') {
			$getReviewEssayIDsSql .= " and u.childClass=".$childclass."";
		}
		if($childsection!='ALL') {
			$getReviewEssayIDsSql .= " and u.childSection='".$childsection."'";
		}
		$getReviewEssayIDsSql .= " order by e.allotedOn asc ";

		//print $getReviewEssayIDsSql->num_rows(); 
		$getReviewEssayIDsSqlValue = $this->dbEnglish->query($getReviewEssayIDsSql);
		if($getReviewEssayIDsSqlValue->num_rows() > 0){
			$reviewEssayIDsRes = $getReviewEssayIDsSqlValue->result_array();
			$i=0;
			foreach($reviewEssayIDsRes as $row){
				$essayID=$row['essayID'];
				
				$getEssayDetailSql=$this->dbEnglish->query("SELECT em.essayTitle, ed.startTime, ed.timeTaken, ed.topicID FROM essayDetails ed, essayMaster em WHERE ed.topicID = em.topicID and ed.essayID=$essayID");
				if($getEssayDetailSql->num_rows() > 0){
					$essayDetailArr = $getEssayDetailSql->row();
					$this->returnArr[$i]['essayID']     	= $row['essayID'];
					$this->returnArr[$i]['essayTitle']  	= $essayDetailArr->essayTitle;
					$this->returnArr[$i]['scoreID']     	= $row['scoreID'];
					$this->returnArr[$i]['Author']      	= $row['childName'];
					$this->returnArr[$i]['Class']       	= $row['childClass'];
					$this->returnArr[$i]['Section']     	= $row['childSection'];
					$this->returnArr[$i]['childclass']  	= $row['childClass'].$row['childSection'];
					$this->returnArr[$i]['isFeedbackSaved'] = ($row['selectFeedback']!="") ? 'Viewed' : 'Unreviewd';
	...
	public function retreiveEvaluatorFeedback($eID,$spellingCheck=false){
		$eID=$eID*1;
		$getEssayIDSql=$this->dbEnglish->query("SELECT * FROM essayScoring WHERE scoreID=$eID");
		if ($getEssayIDSql->num_rows() > 0){
			$essayIDArr = $getEssayIDSql->row();
			$essayID=$essayIDArr->essayID;
			//$getEssayDetailSql=$this->dbEnglish->query("SELECT * FROM essayDetails WHERE essayID=$essayID");
			$getEssayDetailSql=$this->dbEnglish->query("SELECT em.essayTitle, ed.* FROM essayDetails ed, essayMaster em WHERE ed.topicID = em.topicID and ed.essayID=$essayID");
			...
			if ($getEssayDetailSql->num_rows() > 0){
				...
				//if (strpos($returnDataArr->userID, '-') !== FALSE){
					//$returnDataArr->userID = str_replace("-","",$returnDataArr->userID);					
					$getChildClassQuery = "SELECT childClass as class,childName as author FROM userDetails WHERE userID=".$returnDataArr->userID;
				//}
				...
				$returnDataArr->plainEssay=$returnDataArr->userResponse;
				$returnDataArr->title=$returnDataArr->essayTitle;
				...
				{	
					...
					array_push($spellCheckFields,$returnDataArr->userResponse);
				...
				$returnDataArr->spellCheck = $this->session->userdata('spellCheck');
	...
	public function saveReviewedEssayDetails($essayDetailsArr){
		...
		$this->dbEnglish->query("UPDATE essayScoring SET score=".$essayDetailsArr['essayScore'].", feedback='".addslashes($essayDetailsArr['generalFeedback'])."', selectFeedback='".mysql_real_escape_string($essayDetailsArr['essaySpecificFeedback'])."', status=".$essayDetailsArr['status'].$submitE.", rubricSc='".$essayDetailsArr['rubricSc']."', isFeedbackSaved='".$essayDetailsArr['isFeedbackSaved']."' WHERE scoreID=".$essayDetailsArr['essayScoreID']);						
	...
	function setEssayStatus($scID){
		$getEssayDetailSql=$this->dbEnglish->query("SELECT essayID, COUNT(*)-SUM(status) as urv, SUM(status) as rv, 
				...
				FROM essayScoring GROUP BY essayID
				HAVING essayID IN (SELECT essayID FROM essayScoring WHERE scoreID=$scID)");
		...
		if ($getEssayDetailSql->num_rows() > 0){
			...
			$setStatusSql=$this->dbEnglish->query("UPDATE essayDetails SET STATUS=2 WHERE essayID=$esID");
			
			$getWriterSql=$this->dbEnglish->query("SELECT userID FROM essayDetails WHERE essayID=$esID");
	...
	function saveComment($scoreID,$essaySFB){
		$this->dbEnglish->query("UPDATE essayScoring SET selectFeedback='".mysql_real_escape_string($essaySFB)."' WHERE scoreID=".$scoreID);						
	...
	function removeComment($scoreID,$essaySFB){
		$this->dbEnglish->query("UPDATE essayScoring SET selectFeedback='".$essaySFB."' WHERE scoreID=".$scoreID);						
	 ...
	 * */
        function activateNewEssay($essayName,$startDate,$endDate,$grade,$section){
            ...
            $this->dbEnglish->set('topicID', $abbreviation.$lastID);
        ...
        function deactivateOrHideExistingEssay($essayID){
            $updated=false;
                foreach($essayID as $id){                    
                    ...
                    $this->dbEnglish->where('topicID',$id['essayID']);
                    $query = $this->dbEnglish->get();
	 ...
	 * */
        function topicsAssignByTeacher($userID,$childClass,$childSection,$schoolCode){
            $this->dbEnglish->Select('em.topicID, em.topicID as essayID, em.srno, em.essayTitle, tea.activationDate, tea.deactivationDate, tea.forceDeactivateDate,tea.isActive, tea.class, tea.section, tea.schoolCode, em.userID', false);
            $this->dbEnglish->from("teacherEssayActivation tea");
            $this->dbEnglish->join('essayMaster em','tea.topicID = em.topicID','INNER');
            if($childClass!='ALL') :
		...
		$this->dbEnglish->from('essayDetails ed');
                $this->dbEnglish->where('ed.topicID',$valueArr['topicID']);
	 ...
	 * */
        function getSubmissionByTopic($topicID,$childclass,$childsection){
            ...
            $this->dbEnglish->join('essayScoring es','es.essayID = ed.essayID','INNER');
	 ...
	 * */
        function getSubmissionByStudents($topicID,$childclass,$childsection){
            ...
            $this->dbEnglish->join('essayDetails ed','ed.userID = ud.userID AND ed.topicID ="'.$topicID.'"','LEFT');
            $this->dbEnglish->join('essayScoring es','es.essayID = ed.essayID','LEFT');
	 ...
	 * */
        function getEssayChosenByStudent($childclass,$childsection){
            $userID=$this->user_id;
            $schoolCode=$this->school_code;
             $this->dbEnglish->Select('COUNT(ed.topicID) AS \'submissions\',em.essayTitle,ud.childClass class,ud.childSection section, ed.essayID, ed.topicID', false);
            $this->dbEnglish->from("essayDetails ed");
            $this->dbEnglish->join('userDetails ud','ed.userID = ud.userID','INNER');
            $this->dbEnglish->join('essayMaster em','ed.topicID = em.topicID','INNER');
            $this->dbEnglish->join('essayScoring es','es.essayID = ed.essayID','INNER');
            if($childclass!='ALL') :
            	$this->dbEnglish->where('ud.childClass',$childclass);
            endif;
	 ...
	 * */
        function getActiveTopicWithinRange($childclass,$childsection,$startDate,$endDate){
            $userID=$this->user_id;
            $schoolCode=$this->school_code;
            $this->dbEnglish->Select('tea.topicID,tea.topicID as essayID,em.essayTitle, tea.deactivationDate', false);
            $this->dbEnglish->from("teacherEssayActivation tea");
            $this->dbEnglish->join('essayMaster em','tea.topicID = em.topicID','INNER');
