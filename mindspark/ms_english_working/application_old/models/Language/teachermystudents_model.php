	...
	function updateTeacherDetails($data)
	{
		...
		foreach ($data as $key => $valueMainEssay)
		{
			...
			if(isset($valueMainEssay['primaryClass']))
			{
				...
				{
					$getUserIdsql = "select e.scoreID,e.evaluatorID,e.evaluatorLog from essayScoring e join userDetails ud on ud.userID = e.userID where CONCAT_WS('', ud.childClass, ud.childSection) = '$value' and ud.schoolCode=$schoolcode";	
					...
					foreach ($userIdOf as $key => $essayValue) 
					{
						...
						$this->dbEnglish->update('essayScoring', $dataUpdateTeacher);
					}
					...
					/*END*/
                                        ...
                                        $getPrevUserIdsql = "select srno,topicID from teacherEssayActivation where CONCAT_WS('', class,section) = '$value' and schoolCode='".$schoolcode."'";	
					...
					foreach ($getPrevUserIduserIdOf as $key => $essayValue) 
					{
                                                ...
                                                $this->dbEnglish->where('topicID', $essayValue['topicID']);
                                                $query = $this->dbEnglish->get();
