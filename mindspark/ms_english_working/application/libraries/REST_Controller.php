...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
	...
	public function __construct()
	{
		...
		{
			$this->response(array('eiStatus' => 0 ,'eiCode' => 400, 'eiMsg' => 'Only AJAX requests are accepted.'), 505);
		}
	}
	...
	public function _remap($object_called, $arguments)
	{
		...
		{
			$this->response(array('status' => 0, 'eiMsg' => 'Unsupported protocol'), 403);
		}
		...
		if (config_item('rest_enable_keys') AND $use_key AND $this->_allow === FALSE)
		{
			...
			{
				$this->response(array('eiStatus' => 0 ,'eiCode' => 400, 'eiMsg' => 'This API key does not have access to the requested controller.'), 401);
			}
			...
			$this->response(array('eiStatus' => 0 ,'eiCode' => 400, 'eiMsg' => 'Invalid API Key.'), 403);
		}

		// Sure it exists, but can they do anything with it?
		if ( ! method_exists($this, $controller_method))
		{
			$this->response(array('eiStatus' => 0 ,'eiCode' => 400, 'eiMsg' => 'Unknown method.'), 404);
		}
		...
		if (config_item('rest_enable_keys') AND !empty($this->rest->key))
		{
			...
			{
				$this->response(array('eiStatus' => 0 ,'eiCode' => 400, 'eiMsg' => 'This API key has reached the hourly limit for this method.'), 401);
			}
			...
			$authorized OR $this->response(array('eiStatus' => 0 ,'eiCode' => 400, 'eiMsg' => 'This API key does not have enough permissions.'), 401);
		}
	 ...
	 * Response
	 *
	 * Takes pure data and optionally a eiStatus code, then creates the response.
	 *
	 * @param array $data
	...
	public function response($data = null, $http_code = null)
	{
		...
		// If data is NULL and not code provide, ei_Msg and bail
	...
	protected function _detect_api_key()
	{
		...
		// 
		if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $_SERVER[$key_name]))
		{
	...
	{
		$eiStatus = $this->rest->db->insert(config_item('rest_logs_table'), array(
	...
	/**
	 * Retrieve the validation ei_Msgs.
	 *
	 * @return array
	 */
	public function validation_ei_Msgs()
	{
		$string = strip_tags($this->form_validation->ei_Msg_string());
	...
	protected function _perform_ldap_auth($username = '', $password = NULL)
	{
		...
		if ($ldapconn) {
			...
			if ($ldapbind) {
				...
				log_message('eiMsg', 'LDAP Auth: bind unsuccessful');
				return false;
			}
		...
		if (($res_id = ldap_search( $ldapconn, $ldapbasedn, "uid=$username")) == false) {
			log_message('eiMsg', 'LDAP Auth: User ' . $username . ' not found in search');
			return false;
		}

		if (ldap_count_entries($ldapconn, $res_id) != 1) {
			log_message('eiMsg', 'LDAP Auth: failure, username ' . $username . 'found more than once');
			return false;
		}

		if (( $entry_id = ldap_first_entry($ldapconn, $res_id))== false) {
			log_message('eiMsg', 'LDAP Auth: failure, entry of searchresult could not be fetched');
			return false;
		}

		if (( $user_dn = ldap_get_dn($ldapconn, $entry_id)) == false) {
			log_message('eiMsg', 'LDAP Auth: failure, user-dn could not be fetched');
			return false;
		}

		// User found, could not authenticate as user
		if (($link_id = ldap_bind($ldapconn, $user_dn, $password)) == false) {
			log_message('eiMsg', 'LDAP Auth: failure, username/password did not match: ' . $user_dn);
			return false;
		}
	...
	//	mprd($this->input->server('PHP_AUTH_DIGEST'));
		...
		// The $_SESSION['eiMsg_prompted'] variable is used to ask the password
		...
		$valid_pass=$this->checkpass($digest['username']);
...
// mprd($valid_response);
	...
	public function checkpass($username)
	{
		...
		$this->db->where('eeiStatus','Active');
		$this->db->select('vPassword');
		$userdata=$this->db->get('api_keys');
		if ($userdata->num_rows() > 0) 
		{
			$userdata=$userdata->row_array();
			return $userdata['vPassword'];
		}
		else
		{
			return FALSE;
	...
	protected function _check_blacklist_auth()
	{
		...
		{
			$this->response(array('eiStatus' => 0 ,'eiCode' => 400, 'eiMsg' => 'IP Denied'), 401);
		}
	}
	...
	protected function _check_whitelist_auth()
	{
		...
		{
			$this->response(array('eiStatus' => 0 ,'eiCode' => 400, 'eiMsg' => 'Not authorized'), 401);
		}
	}
	...
	protected function _log_access_time()
	{
		...
		return	$this->db->update(config_item('rest_logs_table'),$payload, array('id' => $this->_insert_id));
		// mprd($this->rest->db->update(config_item('rest_logs_table'), $payload, array('id' => $this->_insert_id)));
