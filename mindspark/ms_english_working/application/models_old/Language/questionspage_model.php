...
Class Questionspage_model extends MY_Model
{
	...
	function getNextPassageQuestion($sessionID,$userID,$passageID)
	{
		...
		if($this->session->userdata('currentContentType')==currContentTypePsgConst && ($this->session->userdata('refID') == 0 || $this->session->userdata('refID') == '')):
			$sessionTypeToShow=$this->studentsessionflow_model->getNextContentType($userID);
			$this->session->set_userdata('sessionTypeToShow',$sessionTypeToShow);
			$this->getsessionFlowData($sessionID,$userID,$sessionTypeToShow);
		endif;
	...
	function getsessionFlowData($sessionID,$userID,$passageID=null,$sessionTypeToShow="")
	{
		...
		elseif(strtolower($sessionTypeToShow) == contentFlowSpeaking || strtolower($sessionTypeToShowSession) == contentFlowSpeaking){	
	...
	/**
	 * function role : check if there is any conversation pending to be attempted. If yes the return the refID else return false
	 * param1 : userID
	 * @return : either refID or false
	 * 
	 * */
	...
	function getUserAttemptedPassageQuestions($userID,$passageID)
 	{		
		...
		return $result;
     	...
     	/**
	 * function role : set speaking question completed status in userCurrentStatus
	 * param1 : userID
	 * param2 :speakingAttemptedQuestions
	 * @return  string, list of attempted qcodes , empty string if there are no questions attempted
	 * 
	 * */
	...
	/**
	 * function role : increments the totalAttempts in userContentAttemptDetails table
	 * param1 : userID
	   param2 : contentType
	 * */
	...
	/**
	 * function role : increments the totalAttempts in userContentAttemptDetails table and userContentFlowStatus table
	 * */
	 public function updatePsgTotalAttemptCount(){	 	
		...
		$this->session->unset_userdata('isContentExhaust');
	...
	*/
	...
	function correctAccLogMismatchCount(){
		...
		foreach($contentTypeArr as $key=>$value)
		{
			...
			{
				if(($value['contentType'] == readingContentTypeConst && !$this->session->userdata('isReadingContExhaust')) || ($value['contentType'] == listeningContentTypeConst)){
					...
					if($value['quesPsgAttemptCount']!=count($this->currFreeQuesScoreIDPsgArr)){
