...
Class Questionspage_model extends MY_Model
{
	...
	{	
			...
			if(!$isPsgQnsPending && ($cct =="passage_ques"))         // check if  passage_ques of attempted passage is not set in userCurrentStatus
				{
					...
					//print_r($passageIDArr);exit;
					//$this->setNextSessionPassages($userID,$prevrefID=NULL);
					...
					/*if($sessionTypeToShow=='Passage'):
						$currentContentType='passage';
					
					
					$data=array('currentContentType'=>$currentContentType,'refID'=>$passageIDArr['qcodePassageID']);
					$this->session->set_userdata($data);
					endif;*/
				}
		}
		...
		if($isPsgQnsPending){
			...
			//$getUserContentFlowOrder = 0;
			//$sessionTypeToShow=$this->studentsessionflow_model->getSessionTypeToShow($userID);
			$sessionTypeToShow=$this->studentsessionflow_model->getNextContentType($userID);
			//echo $sessionTypeToShow;exit;
			//echo $sessionTypeToShow;exit;
			//$sessionTypeToShow=$this->studentsessionflow_model->getSessionTypeToContentFlowShow($userID,$getUserContentFlowOrder);

			// Content Logic : Next get next flow order

			// echo '<pre>'; print_r($sessionTypeToShow); exit;
	...
	function getsessionFlowData($sessionID,$userID,$passageID=null,$sessionTypeToShow="")
	{
		...
		//echo $this->session->userdata('presentContentType');exit;
		//echo $currentContentType;exit;
		/*if($currentContentType=='reading'|| $currentContentType=='conversation'):
			$currentContentType='passage';
		endif;*/
		...
		{	
			...
			if($currentContentType != "N/A"){
				 ...
				 // content logic : Existing user nextflow content ->based on the current order to next order 
				//echo '<br/> my test 2';
	...
	function setCurrentPsgQuestions($userID,$passageID=false,$chkAttemptedQues=false){
		...
		if($chkAttemptedQues){
			//if($resultArr['remediationStatus']==1):
				/*$attemptedPassageQuestionsArr = array(0=>'');
				if($chkAttemptedQues){
			...
			//print_r($attemptedPassageQuestionsArr);echo "attemptedPassageQuestionsArr";exit;
...
//echo $this->dbEnglish->last_query();
	...
	function savePassageAttempts($userID,$passageID,$currentPassagePart,$complete,$timeSpent,$sessionID)
	{
		...
		if(count($checkPassageAttempt)>0)
		{
			...
			if($previousAttemptCount['completed']==2):
				$insertData=array(
					...
					'attemptCount'		=>	$previousAttemptCount['attemptCount']+1

				);
				//print_r($insertData);exit;
				$this->dbEnglish->insert('passageAttempt', $insertData); 
				$this->passageAttemptID = $this->dbEnglish->insert_id();
				echo $this->dbEnglish->last_query();echo  "record found";
				//exit;
			else:
				//echo $currentPassagePart;exit;
				$data = array(
					   'currentPassagePart' => $currentPassagePart,
					   'completed' => $complete,
					   'totalTime' => $checkPassageAttempt[0]['totalTime'] + $timeSpent,
				...
				$this->dbEnglish->where('userID', $userID);
				$this->dbEnglish->where('attemptCount', $previousAttemptCount['attemptCount']+1);
				$this->dbEnglish->where('passageID', $passageID);
				//exhaustion
				/*if(($psgContentType == "Textual" || $psgContentType == "Illustrated") && ($this->session->userdata('isReadingContExhaust')))
					$this->dbEnglish->where('exScoringID',$exScoringID);
				...
				echo $this->dbEnglish->last_query();echo "updation";
		...
		else
		{
			...
			$this->passageAttemptID = $this->dbEnglish->insert_id();
			echo $this->dbEnglish->last_query();echo "record not found";exit;
		}
		if($complete){
			$currentPsgQuestions = $this->session->userdata('currentPsgQuestions');
	...
	{
		$psgContentType=$this->getPassageType($passageID);
		$remediationstatus=$this->getRemediationStatus($userID,$passageID);
		//change the condition-testing
		if($remediationstatus['remediationStatus']==1):
			$attemptCount=$this->getMaxAttemptCountForQuestion($userID,$passageID);
			$this->dbEnglish->Select('qcode');
			$this->dbEnglish->from($this->questionAttemptClassTbl);
			$this->dbEnglish->where('userID',$userID);
			$this->dbEnglish->where('passageID',$passageID);
			$this->dbEnglish->where('attemptCount',$attemptCount['maxCount']);
			$query = $this->dbEnglish->get();
			...
			//print_r($attemptedPassageQues[0]['qcodes']);exit;
		...
		if($this->dbEnglish->affected_rows() == 1){
			if($qcodePassageID!=0){
				...
				if($isLastCompletedPsgQnsPending == 0)
				{
					...
					//print_r($this->session->userdata('contentFlowMaster_1Type'));exit;
					/*END*/
...
/*
	...
	function saveNCQQuestionsResponse($userID,$qcode,$questionNo,$timeTaken,$timeTakenExpln,$correct,$userResponse,$questionType,$sessionID,$skillID,$subSkillID)
	{
		...
		if($this->dbEnglish->affected_rows() == 1){
			 ...
			 $contentFlowQuantity=$contentFlowQuantityArr['contentQuantity'];
			 $contentAttemptCount=$this->session->userdata('contentAttemptCount');
			 ...
			 //echo $contentFlowQuantity;
			// echo $contentAttemptCount;
