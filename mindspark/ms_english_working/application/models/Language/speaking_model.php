...
Class Speaking_model extends MY_Model
{
	...
	public function setNextSessionSpeakingQuestions($userID){
		...
		$unattemptedSpkngIDArr= $this->unattemptedspeakingQcode($contentType,$attemptedSpkngID,$msLevel);
		...
		if(isset($unattemptedSpkngIDArr) && count($unattemptedSpkngIDArr)) :
			...
			$this->studentsessionflow_model->updateUserCurrentStatusTbl($userID);
		else :
	...
	public function unattemptedspeakingQcode($contentType,$attemptedSpkngID,$msLevel) 
	{
		$this->dbEnglish->Select('qcode');
		$this->dbEnglish->from('questions');
		$this->dbEnglish->where('qTemplate',$contentType);
	...
	public function speakingLowestAttempts($userID) {
		...
		$subquery = "(SELECT if(($speakingAttemptCount)!=1,qcode,null) as qcode FROM speakingAttempts where attemptCount=($speakingAttemptCount) and userID=$userID) AS b";
