...
Class Questionspage_model extends MY_Model
{
	...
	*/
	...
	function updateContentAttemptLog(){
		$userID = $this->session->userdata('userID');
		$bunchDetailsSQL = "SELECT currentBunchID,totalAttempts,bunchCompleted from educatio_msenglish.userContentAttemptDetails where userID=$userID and contentType = 'free_ques'";
		...
		if($bunchCompleted == 1){//i
			...
			$this->dbEnglish->where('userID',$userID);
			$this->dbEnglish->where('contentType','free_ques');
			...
			$totalAttemptsSQL = "SELECT totalAttempts FROM educatio_msenglish.userContentAttemptDetails where userID=$userID and contentType = 'free_ques'";
 ...
 function checkPassageCompleted($questionAttemptTable, $userID, $passageID, $scoringID, $chkForAdaptive=true)
	...
	function isRedirectToEssayWriter($userID){
		...
		if($checkNewuserSession && $this->category == 'STUDENT') :
			$isRedirectToEssayWriterSQL = "SELECT count(*) as cnt FROM educatio_msenglish.essayDetails WHERE userID = $userID AND lastModified BETWEEN (date_sub(now(),INTERVAL 15 DAY)) AND (now())";
