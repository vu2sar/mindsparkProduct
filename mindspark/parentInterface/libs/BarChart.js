...
//											   var _value = formatFloat((row[ci].v / _total) * 100);
                                                                                            var _value = formatFloat(row[ci].v) ;
											   if(_value <= 0)
											   		continue;
										...
										if(_svgSupport){
											__parent__.append(addTextNode({x:pos.x, y:pos.y, stroke:'none','font-family':"Arial", fill:'black', 'font-size':10 }, _value + "%", _el))
										}else{
											__parent__.get(0).appendChild(addVmlTextNode({left:pos.x, top:pos.y, width:'40pt',height:'20pt', strokecolor:'none','font-family':"Arial", fillcolor:'black', 'font-size':10 }, _value + "%", _el))
										}
									}
						}
	  				};
