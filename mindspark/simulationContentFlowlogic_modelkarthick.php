...
Class simulationContentFlowlogic_model extends CI_Model{
	...
	public function GetContentFlowOrder($userID,$currentContentType){
		$selectContentFlow=$this->dbEnglish->query("SELECT contentType,contentQuantity,contentOrder FROM contentFlowMaster where criteria='N/A' order by contentOrder asc");
						...
						if($checkPsgQuesAttempt){
                                          	...
                                          	$userLastAttemptArr  = array();
                                           	...
                                           	if($curContentType=='passage'){
                                                     	...
                                                     	$refCodeDetails =$this->getQcodePassageDetails($curRefID);
                                                     	if($refCodeDetails['qcodePassageID']!=0){$currentPassageID = $refCodeDetails['qcodePassageID'];
                                                                	$checkPsgQuesAttempt=$this->userAttemptedPsgQues($currentPassageID,$userID);
                                                                	if($checkPsgQuesAttempt){    
                                                                           		$userLastAttemptArr[0]='NA';    
                                                                           		$userLastAttemptArr[1]=$tmpLastAttempt;           
	...
	echo'<tr>'; 
      ...
      //  if($userInformation['currentContentType']=='free_ques') :
	...
	//------------------------------------- Pending Qcodes Flow --------------------------------//
		...
		$pending_qcode = "";
		//echo $currentBunchingData;exit;
		$nextQcodesToBeAttempted=$this->nextQcodesToBeAttempted($userID,$userInformation,$this->questionAttemptClassTbl,$newIndex,$userLastAttemptArr[0],$userLastAttemptArr[1],$currentBunchingID,$currentBunchingData);
		// echo '<pre>'; print_r($nextQcodesToBeAttempted); exit;
		...
		echo "</tr>";
	        	...
	        	<tr>
	        		...
	        		elseif($value=='C'):
	        			...
	        			$nextQcodesToBeAttempted=$this->nextQcodesToBeAttempted($userID,$userInformation,$this->questionAttemptClassTbl,$currentAtmptIndex,$userLastAttemptArr[0],$userLastAttemptArr[1],$currentBunchingID,$currentBunchingData);
...
//endif;
	...
	public function nextQcodesToBeAttempted($userID,$userInfo,$qstnAtmptClss,$newIndex,$lastAttmpt,$currentAttempt,$currentBunch,$finalbunchFlowArr){		
		if($userInfo['currentContentType']=='passage_ques'):
			$passageId= $this->getPsgIdFromRefID($userInfo['refID']);
			$getPassageId=$passageId[0]->passageID;
		elseif($userInfo['currentContentType']=='passage'):
			$getPassageId=$userInfo['refID'];
		elseif($userInfo['currentContentType']=='free_ques'):
    ...
    print_r($currentBunchingCount);exit;
	...
	function getPsgAtmptdCount($psgWithMinAttmptCount,$userID){
		...
		$result=$this->dbEnglish->query($query)->result_array();
		     	//echo $result[0]['maxAttemptCount'];exit;
	}
...
function nextFreeQuestion($questionAttmptTbl,$childClass,$userID,$currentBunchingFlow){
	...
	function getMaxContentOrders(){
	    $contentId=$this->dbEnglish->query("SELECT contentType,contentQuantity FROM contentFlowMaster where criteria = 'N/A' order by contentOrder ASC ");
