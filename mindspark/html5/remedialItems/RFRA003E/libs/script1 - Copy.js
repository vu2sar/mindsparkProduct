...
division.prototype.createGrid = function () {
    ...
    if (ind != -1) {
        for (var i = 0; i < (this.answer.toString().length - ind - 1); i++) {
            ...
            if (i == 0) {
                $("#row3 #coloum" + (5 + this.dividendArray.length - 1 + i)).html('<input type="text" class="systemTextBox" data="0" value="0">');
            }
            else {
                $("#row3 #coloum" + (5 + this.dividendArray.length - 1 + i)).html('<input type="text" class="systemTextBox" data="' + this.answerArray[answerIndex++] + '" value="' + this.answerArray[answerIndex++] + '">');
            }
        }
    }
...
division.prototype.createTextBoxes = function () {
    ...
    for (var i = index; i < (this.answerArray.length + index); i++) {
        ...
        $("#row2 #coloum" + i).html('<input type="text" class="inputBox quotientTextBox" data="' + this.answerArray[answerIndex] + '" value="' + this.answerArray[answerIndex] + '">');
        answerIndex++;
    ...
    if (this.divisor <= greater) {
        ...
        if (this.dividendArray[dividendPointer + 1] != undefined) {
            ...
            $("#row" + (rowCounter) + " #coloum" + (staticColoum + dividendPointer)).html('<input type="text" class="inputBox valueTextBox validate" data="' + parseInt(this.dividendArray[dividendPointer + 1]) + '" value="' + parseInt(this.dividendArray[dividendPointer + 1]) + '">');
        }
        else {
            greater = parseInt(greater + '' + parseInt(0));
            $("#row" + (rowCounter) + " #coloum" + (staticColoum + dividendPointer)).html('<input type="text" class="inputBox valueTextBox validate" data="' + this.answerArray[answerIndex] + '" value="' + this.answerArray[answerIndex] + '">');
        }
        dividendPointer++;
        smaller = this.divisor;
    }
    ...
    for (var i = startIndex; i < this.answerArray.length; i++) {
        if (this.answerArray[i].indexOf('.') == -1) {
            ...
            result = greater - multiplyResult;
        }
        else {
            multiplyResult = (this.divisor * (this.answerArray[i].toString().substring(1, this.answerArray[i].toString().length)));
            result = greater - multiplyResult;
        }
        result += '';
        ...
        for (var j = 0; j < createLength; j++) {
            $("#row" + (rowCounter) + " #coloum" + (coloumCounter + j)).html('<input id="' + j + '" type="text" class="inputBox valueTextBox validate" data="' + multiplyResult.toString()[j] + '" value="' + multiplyResult.toString()[j] + '">');
            setFlow.push("#row" + (rowCounter) + " #coloum" + (coloumCounter + j) + ' input');
        }
        rowCounter++;
        for (var j = createLength - 1; j >= 0; j--) {
            $("#row" + (rowCounter) + " #coloum" + (coloumCounter + j)).html('<input id="' + j + '" type="text" class="inputBox resultTextBox validate" data="' + result.toString()[j] + '" value="' + result.toString()[j] + '">');
            setFlow.push("#row" + (rowCounter) + " #coloum" + (coloumCounter + j) + ' input');
        }
        ...
        if (this.divisor <= greater) {
            ...
            if (this.dividendArray[dividendPointer + 1] != undefined) {
                greater = parseInt(greater + '' + parseInt(this.dividendArray[dividendPointer + 1]));
                $("#row" + (rowCounter) + " #coloum" + (coloumCounter + 1)).html('<input type="text" class="inputBox valueTextBox validate" carry="true" data="' + parseInt(this.dividendArray[dividendPointer + 1]) + '" value="' + parseInt(this.dividendArray[dividendPointer + 1]) + '">');

                setFlow.push(("#row" + (rowCounter) + " #coloum" + (coloumCounter + 1) + " input"));
            }
            else {
                greater = parseInt(greater + '' + parseInt(0));
                $("#row" + (rowCounter) + " #coloum" + (coloumCounter + 1)).html('<input type="text" class="inputBox valueTextBox validate" carry="true" data="0" value="0">');
    ...
    $("#row3 #coloum4").html('<input type="text" class="systemTextBox validate" disabled value="' + this.dividendArray[0] + '" data=' + this.dividendArray[0] + ' value=' + this.dividendArray[0] + '>');
    for (var i = 5; i < 5 + this.dividendArray.length - 1; i++) {
        $("#row3 #coloum" + (i)).css({ 'border-top': '2px solid' });
        $("#row3 #coloum" + (i)).html('<input type="text" class="systemTextBox validate" disabled value="' + this.dividendArray[i - 4] + '" data=' + this.dividendArray[i - 4] + ' value=' + this.dividendArray[i - 4] + '>');
    }
