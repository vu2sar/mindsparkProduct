$(document).ready(function(){
    ...
    $('#whatsGoing').on('show.bs.modal', function (e) {   
        $('#whatsGoing').show();     
        var positiveHeight = $(".whats-going-positive").height();        
        $(".no-concern-message").css("min-height",positiveHeight+"px");
        var negativeHeight = $(".whats-going-negative").height();        
        $(".no-positive-message").css("min-height",negativeHeight+"px");
    });
...
var miscon = 0;
    angular.module('mindspark', [
        ...
        $scope.attachEvent = function(){
            ...
            $(window).scroll(function () {
                ...
                else if((($(".common_wrong_answer").offset().top - 65) - $(window).scrollTop()) < 15 && (($(".common_wrong_answer").offset().top - 65) - $(window).scrollTop()) > -210)
                {
                ...
                else if((($(".learning_unit").offset().top - 65) - $(window).scrollTop()) < 400)                
                {
                ...
                if((($(".assessment_details").offset().top - 65) - $(window).scrollTop()) < 15 && (($(".assessment_details").offset().top - 65) - $(window).scrollTop()) > -310 ) {
        ...
        $scope.pageTrack = function(page){
            ...
            if(typeof $scope.topic_info !== 'undefined')
            {
    ...
    }).directive('studentProgressAssessment',function($timeout){
        return {
            transclude:true,
            restrict : 'E',
            scope : {
                ...
                '<div class="time-to-complete-card progress-card">',
                    ...
                    '<div class="card-body">',
                        '<div>',
                            ...
                            '<button class="card-footer icon pull-right" data-toggle="tooltip" data-placement="left" title="" data-original-title="Estimated from previous attempts" data-trigger="focus">',                           
                            '</button>',                       
                        '</div>',                        
                    '</div>',                    
        ...
        service.request = function(mode,params,callback){
            $timeout(function(){
                ...
                $http({
                    ...
                    {
                        window.location.href = '/mindspark/teacherInterface/logout.php';
                    }
                    else
                        callback(response);
                });
...
function finished_assessent(element){
    $(element).parent().parent().removeClass('loader-frame');
    var iframe = $(element)[0];
    var cwa = $($(iframe.contentWindow.document.body)[0]).find('.cwa');
    $.each(cwa, function() {
            ...
            var details =   $(currentElement).attr("id");
            var detailsInput =   $(currentElement).next("input");
            $.post("ajaxRequest.php","mode=questionForClassDiscsussion&quesDetails="+detailsInput.val(),function(data) {                 
                $(currentElement).html(data);
        });
    });    
}
