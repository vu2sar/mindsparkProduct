...
$(function(){
	...
	//Session ID дл�? Flash-загрузчика
	var SID;
	$.ajax({
		type: "POST",
		url: "connector/php/",
		...
		$('#newFolderBlock').slideDown('fast', function(){
			$('#newFolderBlock input').focus().blur(cancelNewFolder).keypress(function(e){
				if(e.which == 13) {
					...
					//Значит в�?е верно: a-Z0-9-_ и управление вводом
				} else {
					return false;
				}
	...
	function saveNewFolder(){
		...
		$('#menuCancelFolder,#menuSaveFolder').hide();
		$('#menuCreateFolder').show();
		//Запро�? на �?оздание папки + �?ервер должен отдать новую �?труктуру каталогов
