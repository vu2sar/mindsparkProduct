...
class Editor extends CI_Controller {
	...
	public function upload($lang='english')
	{
		...
		if ($this->upload->do_upload())
		{
			...
			require_once $_SERVER['DOCUMENT_ROOT'].'/mindspark/constants.php';
			require_once $_SERVER['DOCUMENT_ROOT'].'/mindspark/parentApp/constants.php';
			if(!class_exists('S3'))
				require_once $_SERVER['DOCUMENT_ROOT'].'/s3/S3.php';
			$s3 = new S3(awsAccessKey, awsSecretKey);
			$destination = "teacher_images/".time().'_'.$result['file_name'];
			$s3->putObjectFile($result['full_path'], PROFILE_PIC_BUCKET, $destination, S3::ACL_PUBLIC_READ);
			...
			$result['file_name'] = CLOUDFRONT_URL . $destination;
