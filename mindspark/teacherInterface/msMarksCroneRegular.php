<?php
    ...
    $counter=0;
    $schoolArr=array(1752);
    $arrlength=count($schoolArr);
    for($x=0;$x<$arrlength;$x++){
        $querySchool="SELECT distinct schoolCode,order_id,start_date,end_date,B.schoolname FROM educatio_educat.ms_orderMaster A, educatio_educat.schools B WHERE A.schoolCode=B.schoolno AND order_type='regular' AND A.year=2013 AND A.schoolCode!=2387554 AND A.schoolCode=".$schoolArr[$x]." ORDER BY city";
        $schoolResult = mysql_query($querySchool) or die(mysql_error());
            ...
            while($usersLine=mysql_fetch_array($usersResult))
                ...
                if($stud!=0){
                        foreach ($msMarks as $userID=>$marks)
		                {
                            $cnt++;
                            $cnt2++;
                            $insertQuery="INSERT INTO msMarksTable(childName,schoolName,childClass,msMarks,noOfStudents,schoolCode) Values('".$userArray[$userID]."','".$schoolNameArr[$userID]."','".$classArray[$userID]."',".$marks.",".$stud.",".$schoolCode.")";
