<?php
    ...
    function runCroneforSchool($schoolCode,$orderID,$start_date,$end_date,$schoolname){
        $upgradeQuery="SELECT is_upgrade FROM educatio_educat.upgrdation_conformation WHERE schoolCode=".$schoolCode;
        ...
        $counter++;
         ...
         //echo '<table border="1"><tr><td>Sr no</td><td>Child name</td><td>School</td><td>Class</td><td>MS marks</td><td># of students</td><td>schoolcode</td>';
        ...
        while($orderLine=mysql_fetch_array($schoolOrder)){
            ...
            //echo $orderLine[0];
            //$query = "SELECT userID,username,childName, childClass, childSection, childDob, classChangeHistory,schoolName FROM adepts_userDetails WHERE schoolCode=".$schoolCode." AND childClass='".$classCnt."'";
            //$query .= " AND category='STUDENT' AND subcategory='School' AND endDate>='".$_GET['to']."'  ORDER BY childSection, childName";
            $query="select distinct A.userID,B.childSection,B.childName from adepts_sessionStatus_archive A,adepts_userDetails B WHERE B.schoolCode=$schoolCode and A.userid=B.userid and A.class=".($classCnt)." and A.startTime<='$end_date' AND startTime>='$start_date'";
            ...
            while($usersLine=mysql_fetch_array($usersResult))
		        {
                    ...
                    $tableExtensionArr[$usersLine['userID']]=1;
			        //if($classChange=='2014'){
           //             $tableExtensionArr[$usersLine['userID']]=1;
           //         }
           //         else
           //             $tableExtensionArr[$usersLine['userID']]=0;
                    $msMarks[$usersLine['userID']]=0;
                    $msMarks[$usersLine['userID']] = explode("~",newScore($usersLine['userID'], $_GET['from'],$_GET['to'], $classCnt,0,$tableExtensionArr[$usersLine['userID']]));
                ...
                $cnt=0;
                arsort($msMarks);
                $topFivePer=round($stud*0.05,0);
                if($stud!=0){
                        //echo "No. Of Students: ".$stud;
                        // echo '<table border="1"><tr><td>Sr no</td><td>Child name</td><td>School</td><td>Class</td><td>MS marks</td>';
    	                foreach ($msMarks as $userID=>$marks)
		                {
                            $cnt++;
                            $cnt2++;
                            //echo '<tr><td>'.$cnt2.'</td><td>'.$userArray[$userID].'</td><td>'.$schoolNameArr[$userID].'</td><td>'.$classArray[$userID].'</td><td>'.$marks.'</td><td>'.$stud.'</td><td>'.$schoolCode.'</td>';
                            $insertQuery="INSERT INTO msMarksTable(childName,school,childClass,msMarks,noOfStudents,schoolCode) Values(".$userArray[$userID].",'".$schoolNameArr[$userID]."','".$classArray[$userID]."',".$marks.",".$stud.",".$schoolCode.")";
		                    if($cnt==$topFivePer)
                                break;
		                }
                        //echo '<table><br><br>';     
         ...
         for($classCnt=1;$classCnt<11;$classCnt++){
