...
public function getResults($conversationIDs, $checkForPermission = false)
{
	...
	while ($row = $result->nextRow()) {
		...
		$row["replies"] = max(0, $row["countPosts"] - 1);
                $fullNameQuery = ET::$database->query("SELECT a.childName   FROM educatio_adepts.adepts_userDetails a WHERE a.username='$row[lastPostMember]'");
               $row["lastPostMemberFullName"] = $fullNameQuery->result();
		$results[] = $row;
