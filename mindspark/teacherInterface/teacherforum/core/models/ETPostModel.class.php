...
public function getWithSQL($sql)
{
	...
	while ($post = $result->nextRow()) {
                ...
                //Get full name to display instead of username
               $fullNameQuery = ET::$database->query("SELECT a.childName FROM educatio_adepts.adepts_userDetails a  WHERE a.username='$post[username]'");
               $post["fullName"] = $fullNameQuery->result();
			   //Get edit member full name
			   if($post['username']=$post['editMemberName']) {
				   $post["editMemberFullName"] = $post["fullName"];
			   } else {
					$editMemberNameQuery = ET::$database->query("SELECT a.childName FROM educatio_adepts.adepts_userDetails a WHERE a.username='$post[editMemberName]'");
					$post["editMemberFullName"] = $editMemberNameQuery->result();             
			   }
