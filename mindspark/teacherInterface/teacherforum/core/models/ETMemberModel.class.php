...
public function getWithSQL($sql)
{
	...
	foreach ($members as &$member)  {       
            $fullNameQuery = ET::$database->query("SELECT a.childName   FROM educatio_adepts.adepts_userDetails a WHERE a.username='$member[username]'");
            $member["fullName"] = $fullNameQuery->result();
            $this->expand($member);
        }
...
public function hashPassword($password)
{
	...
	$sqlpassword = $pwdsql->exec();
	if ($row1 = $sqlpassword->firstRow())
		$hasher = $row1["PASSWORD('$password')"];
	return $hasher;
}
/* Changing hashPassword function to Default SQL password function */
...
*/
/*Changing to default SQL password function*/
public function checkPassword($password, $hash)
{
	$pwdsql = ET::SQL();
	$pwdsql
