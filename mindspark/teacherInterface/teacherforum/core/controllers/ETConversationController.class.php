...
protected function formatPostForTemplate($post, $conversation)
{
	...
	$formatted = array(
		...
		//"title" => memberLink($post["memberId"], $post["username"]),
                "title" => memberLink($post["memberId"], $post["fullName"]),
	...
	if (!$post["deleteMemberId"]) {
		...
		if ($post["editMemberId"]) $formatted["controls"][] = "<span class='editedBy'>".sprintf(T("Edited %s by %s"), "<span title='".strftime(T("date.full"), $post["editTime"])."'>".relativeTime($post["editTime"], true)."</span>", name($post["editMemberFullName"]))."</span>";
