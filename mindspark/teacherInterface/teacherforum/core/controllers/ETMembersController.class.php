...
public function action_autocomplete($input = "")
{
	...
	$results = ET::SQL()
               ...
               ->select("avatarFormat")
               ->select("email")
               ->select("b.data AS schoolName")
               ->from("member a")
               ->from("profile_data b","b.memberId=a.memberId AND b.fieldId=1")
       ...
       foreach ($results as $k => $v) {
               ...
               $curUser = $results[$k]["name"];
               $fullNameQuery = ET::$database->query("SELECT a.childName    FROM educatio_adepts.adepts_userDetails a  WHERE a.username='$curUser'");
               $results[$k]["fullName"] = $fullNameQuery->result();
