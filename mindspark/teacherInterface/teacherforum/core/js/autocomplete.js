			...
			if (results.length) {
				...
				for (var i in results) {
					...
					name = name.replace(regexp, "<strong>$1</strong>");
                                        var schoolName = $("<div/>").text(results[i].schoolName).html();
                                        var fullName = $("<div/>").text(results[i].fullName).html();

					// Create an <li> for the result and add some event handlers.
					item = $("<li><a href='#'><i>"+results[i].avatar+"</i> "+fullName+" , "+schoolName+"</a></li>").data("position", i).data("member", results[i]).mouseover(function() {
