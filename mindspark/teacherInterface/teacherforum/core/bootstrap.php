...
/*if username session variable matches username in forum database then fetch memberId*/
$sql = ET::SQL();
$sql
    ->select("memberId")
    ->from("member")
    ->where("username",ET::$session->username);
//echo $sql->get(); // Outputs: SELECT `username` FROM `et_member` WHERE `memberId` = 1
$resultMemberId = $sql->exec();

if ($row = $resultMemberId->firstRow())
		$actualMemberId = $row["memberId"];
/*Assign userId session variable to member Id. This is overriding login. It thinks memberId is already logged in*/

if(!empty($_SESSION['username'])) $_SESSION['userId']=$actualMemberId;
