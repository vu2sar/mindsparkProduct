...
public function mentions()
{
	...
	// 	$this->content
	// );
	//Replacing preg_replace with callback function. Display full name instead of username for mentions
	$this->content = preg_replace_callback(
		'/(^|[\s,\.:\]])@([^\s[\]]{2,50})\b/iu',
		function($matches) {
        $fullNameQuery = ET::$database->query("SELECT count(a.childName) as num_row, a.childName FROM educatio_adepts.adepts_userDetails a WHERE a.username='$matches[2]'");
		$result=$fullNameQuery->allRows();
		$fullName='';
		if($result[0]['num_row']>0) $fullName=$result[0]['childName'];
    	if($fullName!=''){
    		return $matches[1]."<a href='".URL('member/name/'.urlencode(str_replace('&nbsp;', ' ', $matches[2])), true)."' class='link-member'>@".$fullName."</a>";
    	}
    	else
    	{
    		return $matches[1]."<a title='Ex-Forum Member, maybe?'>@".$matches[2]."</a>";
    	}
    	},
		$this->content
	);
...
{
	preg_match_all('/(^|[\s,\.:\]])@([^\s[\]]{2,50})\b/iu', $content, $matches, PREG_SET_ORDER);
	$names = array();
