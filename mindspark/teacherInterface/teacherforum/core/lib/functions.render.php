...
{
	if (!$username) $username = "[".T("Ex-Forum Member")."]";
...
function starButton($conversationId, $starred)
{
	...
	// Otherwise, return a clickable star!
		...
		$conversationId = (int)$conversationId;
		$url = URL("conversation/star/".$conversationId."?token=".ET::$session->token."&return=".urlencode(ET::$controller->selfURL));
		return "<a href='$url' class='button big starButton' data-intro='".T("FollowHelp")."' title='".T("Follow to receive notifications")."' data-id='$conversationId'><i class='star icon-star".($starred ? "" : "-empty")."'></i> <span>".($starred ? T("Following") : T("Follow"))."</span></a>";
	}
}
