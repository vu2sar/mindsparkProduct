...
public function init()
{
	...
	if ($this->responseType === RESPONSE_TYPE_DEFAULT) {
		...
		if (!ET::$session->user) {
			...
			$this->addToMenu("user", "blog", "<a href='http://blog.ei-india.com' target='_blank' class='link-login'>".T("EI Blog")."</a>");
		}
		...
		// If the user IS logged in, we want to display their name and appropriate links.
			...
			//$this->addToMenu("user", "user", "<a href='".URL("member/me")."'>".avatar(ET::$session->user, "thumb").name(ET::$session->user["username"])."</a>");
                        $this->addToMenu("user", "user", "<a href='".URL("member/me")."' data-intro='".T("UserHelp")."'>".avatar(ET::$session->user, "thumb").name(ET::$session->user["fullName"])."</a>");
			...
			$this->addToMenu("user", "blog", "<a href='http://blog.ei-india.com' target='_blank' class='link-login' data-intro='".T("BlogHelp")."'>".T("EI Blog")."</a>");
			/*$this->addToMenu("user", "logout", "<a href='".URL("user/logout?token=".ET::$session->token)."' class='link-logout'>".T("Log Out")."</a>");*/
			
			$this->addToMenu("user", "home", "<a href='/mindspark/teacherInterface/home.php?live=2' class='link-login' data-intro='".T("HomeHelp")."'>".T("Home")."</a>");
		}
		...
		if (C("esoTalk.members.visibleToGuests") or ET::$session->user) {
			...
			$stat = "<a href='".URL("members/online")."' class='link-membersOnline' data-intro='".T("OnlineHelp")."'>$stat</a>";
			$this->addToMenu("statistics", "statistic-online", $stat);
		}

		$this->addToMenu("meta", "copyright", "<a href='http://ei-india.com/' target='_blank'>"."Educational Initiatives</a>");
		...
		$this->addJSFile("core/js/intro.js", true);
		$this->addJSFile("config/custom.js", true);
...
public function render($view = "")
{
	...
	if ($this->responseType == RESPONSE_TYPE_DEFAULT and ET::$session->user) {
		...
		$this->addToMenu("user", "notifications", "<a href='".URL("settings/notifications")."' id='notifications' class='button popupButton ".($count ? "new" : "")."' data-intro='".T("NotificationsHelp")."'><span>$count</span></a>");
