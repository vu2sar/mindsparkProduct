...
Class Essay_model extends MY_Model
{
	...
	function saveUserTopic($userID, $title, $msLevel)
	{
		...
		$result = array();
		//$this->dbEnglish->select('essayID, count(*) as count,( select max(srno) from essayMaster ) as srno');
		$this->dbEnglish->select('essayID, e.essayTitle,( select max(srno) from essayMaster ) as srno');
		...
		if(!$isInPredefinedList)    //this topic is not there in predefined list
		{
			...
			if(!$isInUserList) //this topic is not created by this user 
			{
				...
				else
				{
					// Save the custom topic added by student to ews_essayDetails
					$insert_data = array();
					$insert_data['essayID']		= $queryResultPredifined[0]['srno'] + 1;
					$insert_data['essayTitle']  = $title;
					$insert_data['wordLimit']	= 100;
					$insert_data['msLevel']		= $msLevel;
					$insert_data['userID'] 		= $userID;
					...
					$dataUpdate = array(
					   'essayID' => $abbrivation.$lastInsertedId
		            );
					$this->dbEnglish->where('srno', $lastInsertedId);
					$this->dbEnglish->update('essayMaster', $dataUpdate);
			...
			else
			{
				...
				$this->dbEnglish->Select('status, topicID, essay');
				$this->dbEnglish->from('ews_essayDetails');
				...
				else
				{
					if($queryStatusResult['status'] == 0)
					{
						...
						$dataArr['userResponse'] = $queryStatusResult['essay'];
						$dataArr['essayTitle']   = $title; 					// Added by Aditya
						return $dataArr;
	...
	function saveEssayDetails($userID,$essayID,$topic,$userResponse,$status,$sessionID,$timeTaken)
	{
		...
		{
			$this->dbEnglish->Select('srno,userID,essayID');
			...
			$this->dbEnglish->where('userID', $userID);
			$this->dbEnglish->where('essayID', $essayID);
			$this->dbEnglish->where('srno', $queryResultAttempt['srno']);
			$this->dbEnglish->update('essayAttempt', $data);
			return $this->updateEssayWrittingSystem('Save',$userID,$essayID,$userResponse,$status,$timeTaken);
		}
		else
		{
		...
		$attemptedEssayIDsArr = explode(',',$this->getUserAttemptedEssays($userID));
                ...
                if($queryResult[0]['forceEssay']==0){
                    ...
                    foreach ($queryResult as $key => $value) 
                    {
                            $this->dbEnglish->Select('essayID');
                            $this->dbEnglish->from('ews_essayDetails');
                            $this->dbEnglish->where('topicid', $value['essayID']);
                            $this->dbEnglish->where('userID', $userID);
	...
	*
	...
	function updateEssayWrittingSystem($type,$userID,$essayID,$userResponse,$status,$timeTaken)
	{
		...
		if(strpos($serverURL,$this->config->item('staging_url')) !==false){
			$server = 'http://'.$this->config->item('staging_url');
		...
		$essayWritterURL = 'http://13.251.143.110/msengFinalVersion/essaywriting/mseng_essays.php';
	...
	function getEssayName($essayID,$type="",$ews_essayDetailsID="")
	{
		...
		{
			$arr = explode('||',$essayID);
			$ids = sprintf('FIELD(em.essayID, "%s")', implode('","',$arr));
	...
	function fetchEssaySummary($userID)
	{
		...
		$this->dbEnglish->from('ews_essayDetails ed');
		$this->dbEnglish->join('essayMaster em','ed.topicID = em.essayID');
		$this->dbEnglish->where('ed.writerID',$userID);
		$this->dbEnglish->where_in('em.userID',$teacherIDsArr);
	...
	function fetchEssayResponse($userID,$topicID,$essayID)
	{
		...
		$this->dbEnglish->where('ews_essayDetailsID',$essayID);
		$query = $this->dbEnglish->get();
		$queryResult = $query->result_array();
//		echo "<pre>";print_r($this->dbEnglish->last_query());echo "</pre>";
	 ...
	 * */
        function checkIsForceEssay($userID,$topicID){
            ...
            if(isset($queryResult[0]) && !empty($queryResult)){
                $queryResult=$queryResult[0]['essayID'];
            }else{
                $queryResult="no";
            }
            return $queryResult;
	 ...
	 * */
        function showAssignedEssayToStudent($userID){
            $this->dbEnglish->Select('teacherEssayActivation.essayID,teacherEssayActivation.srno, essayMaster.essayTitle,essayMaster.msLevel');
		$this->dbEnglish->from('teacherEssayActivation');
		...
		$queryResult = $query->result_array();
                if(isset($queryResult[0]) && !empty($queryResult)){
                    $this->dbEnglish->Select('ews_essayDetails.essayID,ews_essayDetails.status');
                    $this->dbEnglish->from('ews_essayDetails');
                    $this->dbEnglish->where('ews_essayDetails.userID',$userID);
                    $this->dbEnglish->where('ews_essayDetails.topicID',$queryResult[0]['essayID']);             
