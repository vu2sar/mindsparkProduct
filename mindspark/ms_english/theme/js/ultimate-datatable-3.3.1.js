        ...
        var datatable = {
            ...
            setHideColumn: function(column) {
                if (this.config.hide.active) {
                    var columnId = column.id;
                    var nextColumnId= this.getSimilar(column);
                    if (!this.config.hide.columns[columnId]) {
                        this.config.hide.columns[columnId] = true;
                        this.config.hide.columns[nextColumnId] = true;
                    } else {
                        this.config.hide.columns[columnId] = false;
                        this.config.hide.columns[nextColumnId] = false;
                    }
            ...
            getSimilar: function(column){
                var allColumns=this.getHideColumns();
                var valueId=null;
                $.each(allColumns, function( index, value ) {
                    if(value.id!=column.id && value.extraHeader==column.extraHeader){
                        valueId= value.id;
                    }
                });
                return valueId;
            ...
            //custom sequvence option
            isShowSequence: function() {
                return this.config.showSequence.active;
            },
            //end
...
.run(['$templateCache', function($templateCache) {
  $templateCache.put('udt-table.html',
   ...
   +            '</button>'
   +        '</div>'
   +        '<form name="datatableForm" class="form-inline table-responsive">'
   +            '<table class="v2-detail-report-table table-condensed table-hover " id="v2-detail-report-tbl">'
   +                '<thead>'
   +                    '<tr ng-repeat="(key,headers) in udtTable.getExtraHeaderConfig()">'
   +                        '<th colspan="" class="xheader sr-number">'
   +                            '<span></span>'
   +                        '</th>'
   +                        '<th colspan="{{header.colspan}}" ng-repeat="header in headers" class="xheader {{header.label}}">'
   ...
   +                        '<th ng-if="udtTable.config.showSequence.active"  class="xheader sr-number mainHeader">'
   +                            '<span>S. No.</span>'
   +                        '</th>'
   +						'<th id="{{column.id}}" ng-repeat="column in udtTable.getColumnsConfig()" ng-model="column" ng-if="!udtTable.isHide(column.id)" ng-class="udtTableFunctions.getThClass(column, this)" class="mainHeader">'
   +                            '<span class="header" ng-model="udtTable" ng-click="udtTableFunctions.setOrderColumn(column)" ng-if="udtTable.isShowButton(\'order\', column)" ng-disabled="!udtTable.canOrder()" data-toggle="tooltip" title="{{udtTableFunctions.messages.Messages(\'datatable.button.sort\')}}" ng-bind="udtTableFunctions.messages.Messages(column.header)"/>'
   ...
   +                        '</td>'
   +                         '<td ng-if="udtTable.config.showSequence.active">'
   +                            '{{(value.line.id+1)+(udtTable.config.pagination.pageNumber*udtTable.config.pagination.numberRecordsPerPage)}}'
   +                        '</td>'
   ...
   +        '</form>'
   //customization
   +        '<div class="v2-detail-report-footer">'
   +        '<div class="v2-footer-wrapper">'
   +        '<div class="btn-toolbar col-md-3 col-sm-6 udt-toolbar-results v2-detail-report-total-record" name="udt-toolbar-results" ng-if="udtTable.isShowToolbarResults()">'
   +            '<span class="v2-totla-number-records" disabled="disabled" ng-if="udtTable.config.showTotalNumberRecords">{{udtTableFunctions.messages.Messages(\'datatable.totalNumberRecords\', udtTableFunctions.getTotalNumberRecords())}}</span>'
   +        '</div>'
   +        '<div class="btn-toolbar udt-toolbar-pagination col-md-9 col-sm-6 v2-detail-report-pagination" name="udt-toolbar-pagination" ng-if="udtTable.isShowToolbarPagination()">'
      +            '<div class="btn-group v2-limit">'
   +                '<button data-toggle="dropdown" class="btn btn-default dropdown-toggle v2-change-limit">'
   ...
   +                '</ul>'
   +            '</div>'
   +            '<div class="btn-group v2-pagination" ng-if="udtTable.isShowPagination()">'
   +                '<ul class="pagination">'
...
.run(['$templateCache', function($templateCache) {
  $templateCache.put('udt-toolbar.html',
   ...
   +    '<div class="col-md-12 col-lg-12">'
   +        '<div class="btn-toolbar " name="udt-toolbar-buttons" ng-if="udtTable.isShowToolbarButtons()">'
   +            '<div class="btn-group display_none" ng-switch on="udtTable.config.select.isSelectAll">'
   ...
   +                '</ul>'
   +            '</div>'
   +            '<div class="pull-left v2-usage-details-text col-md-5 col-sm-5">Usage Details</div>'
   +            '<div class="btn-group pull-right col-md-6 col-sm-6" ng-if="udtTable.isShowHideButtons()">'
   +                '<div class="v2-download-excel" id="v2-download-excel">'
   +                     '<button class="btn btn-link" ng-click="exportToExcel(\'v2-detail-report-tbl\');">'
   +                        '<span class="glyphicon glyphicon-download-alt"></span>'
   +                        '</button>'
   +                '</div>' 
   +                '<ul class="nav navbar-nav navbar-right v2-show-heigh-dd-ul">'
   +                     '<li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">+ Add Column(s)</a>'
   +                        '<ul class="dropdown-menu">'
   +                            '<li ng-repeat="column in udtTable.getHideColumns()" class="{{column.extraHeader}} {{column.id}}">'
   +                                '<a href="" ng-click="udtTableFunctions.setHideColumn(column)" ng-switch on="udtTable.isHide(column.id)" >'
   +                                    '<input type="checkbox" class="show-hide-columns" value="1"  ng-switch-when="true">'
   +                                    '<input type="checkbox" class="show-hide-columns" value="1" checked ng-switch-when="false">'
   +                                    '<span ng-bind="udtTableFunctions.messages.Messages(column.extraHeader)"/>'
   +                                '</a>'
   +                            '</li>'
   +                       '</ul>'
   +                   '</li>'
